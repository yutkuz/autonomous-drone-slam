...
async function loadMapsFromApi() {
  ...
  try {
    const response = await fetch("/api/maps", { signal: controller.signal });
    if (!response.ok) throw new Error(`${response.status} ${response.statusText}`);
    const apiMaps = await response.json();
    if (!Array.isArray(apiMaps) || apiMaps.length === 0) return;
    maps = apiMaps.map(normalizeApiMap);
  } catch (error) {
    console.warn("DB API kullanilamadi, statik yedek veri kullaniliyor.", error);
  } finally {
    window.clearTimeout(timeout);
  }
}

function normalizeApiMap(item) {
  ...
  return {
    ...
    period: item.period,
    run: item.relativeRunPath,
    summary: item.summary,
    stats: {
      poses: item.stats?.poses ?? "-",
      links: item.stats?.links ?? "-",
      raw: item.stats?.raw ?? "-",
      voxel: item.stats?.voxel ?? "-",
      coverage: item.stats?.coverage ? `%${item.stats.coverage}` : "-",
      db: item.stats?.databaseSize ?? "-",
    },
    assets: {
      bev: assets.bev || assets.fused || "",
      side: assets.side || "",
      ply: assets.ply || assets.rawPly || "",
      rawPly: assets.rawPly || "",
      meshObj: assets.meshObj || "",
      meshPreviewObj: assets.meshPreviewObj || "",
      meshMtl: assets.meshMtl || "",
      meshGlb: assets.meshGlb || "",
      meshPng: assets.meshPng || "",
      db: assets.db || "",
      front: assets.front || "",
      depth: assets.depth || "",
      left: assets.left || "",
